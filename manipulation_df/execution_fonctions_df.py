...
def fonctions():
    ...
    dff_non_normalise = transformer_donnees(df_jointure, liste1, liste2)
    dff_non_normalise = remplir(dff, lecture_dictionnaire('dic.txt'))
    return liste1, liste2, df, dff, dff_non_normalise

def create_dataframe_criteres():
    liste1, liste2, df, dff, dff_non_normalise = fonctions()
    df_jointure=jointure()
    dataframe = df[['User id']]
    clubs = extraire_colonnes_club(df_jointure)
    clubs.insert(0, 'User id', df['User id'])
    clubs = remplir_common_columns(clubs)
    supprimer_normalisation(df_jointure,df)
    dataframe = pd.merge(dataframe, clubs[['User id', clubs.columns[3]]], on='User id').rename(columns={clubs.columns[3]: 'Club'})
    dataframe = fusionner_donnees(dataframe, dff_non_normalise, liste1, liste2)
...
def tables():
    ...
    df_questions = obtenir_df_final(df_jointure, liste1, liste2)
    df_1 = pd.merge(dataframe, df_jointure, on='User id')
    df_final=pd.merge(df_1, df_questions, on='User id')
    return df_final

def questions_sans_criteres():
    liste1, liste2, df, dff, dff_non_normalise = fonctions()
    df_jointure=jointure()
    df_questions = obtenir_df_final(df_jointure, liste1, liste2)
    return df_questions
